...
export default function UnboxingScanner() {
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.warn("Preferred camera failed, fallback:", err);
  ...
  const startRecording = async () => {
    try {
      ...
      if (currentUid) {
        ...
        if (!userSub || (userSub.end_date && new Date(userSub.end_date).getTime() < Date.now())) {
          setLimitPopup({
            ...
            message: 'Masa aktif paket langganan Anda telah habis. Silakan perpanjang atau upgrade paket di menu Paket Langganan untuk melanjutkan perekaman unboxing.'
          });
          return;
        }
      ...
      alert("Gagal memulai perekaman unboxing.");
    }
  };
  ...
  // Canvas Rendering Loop with Smart Video Rotation
  useEffect(() => {
  ...
  const handleSaveRecording = async () => {
    ...
    console.log(`[UnboxingScanner] Video recorded: ${blob.size} bytes (${mime}) for recId ${recId}`);
      ...
      setTimeout(() => {
        uploadRecordingToDrive(recId, blob, resiToUpload, marketplace).then(async (res) => {
          if (res.success && res.driveFileId) {
            console.log('[UnboxingScanner] Video uploaded to Cloud Server:', res.driveFileId);
            await updateLocalRecordMetadata(recId, { uploadStatus: 'SUCCESS', driveFileId: res.driveFileId });
          } else {
            console.warn('[UnboxingScanner] Cloud upload queued:', res.error);
          }
        }).catch(e => console.warn('[UnboxingScanner] Upload error:', e));
      }, 300);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="bg-surface-container-low border-b border-ui-divider px-1.5 sm:px-3 py-1 flex items-center justify-between gap-1 shrink-0 h-8 sm:h-10">
        ...
        <button
          ...
          className="flex items-center gap-1 bg-secondary/10 text-secondary hover:bg-secondary hover:text-white px-2 py-0.5 rounded text-[11px] font-bold transition-all shrink-0"
        >
      ...
      <section className="flex-1 p-1 sm:p-2 grid grid-cols-1 landscape:grid-cols-12 lg:grid-cols-12 gap-1.5 sm:gap-2 overflow-hidden">
        ...
        <div className="landscape:col-span-8 lg:col-span-8 flex flex-col h-full overflow-hidden">
          ...
          <div 
            ...
            className="relative w-full h-full min-h-0 bg-black rounded-lg border-2 border-secondary overflow-hidden shadow-sm group cursor-crosshair select-none flex items-center justify-center"
          >
            <video 
              ref={videoRef} 
              autoPlay 
              ...
              <>
                ...
                {scanState === 'RECORDING' && (
                  <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 flex items-center gap-1 bg-black/70 backdrop-blur-md px-2 py-0.5 sm:py-1 rounded border border-red-500 shadow-md">
                    ...
                    <span className="font-label-caps text-[10px] sm:text-xs text-white font-bold">UNBOXING REC ● {formatTime(recordingTime)}</span>
                  </div>
                )}
        ...
        <div className="landscape:col-span-4 lg:col-span-4 flex flex-col justify-between gap-1 h-full overflow-y-auto">
          {scanState !== 'IDLE' ? (
            <div className="bg-surface-container-lowest border border-ui-divider rounded-lg shadow-sm p-1.5 sm:p-2 flex flex-col gap-1 animate-[fade-in_0.2s_ease-out]">
              <div className="flex justify-between items-center pb-1 border-b border-ui-divider">
                <div className="flex items-center gap-1">
                  <span className="w-3.5 h-3.5 bg-secondary text-white rounded-full flex items-center justify-center text-[8px] font-bold">U</span>
                  <span className="text-[11px] text-secondary font-bold">Unboxing Pesanan</span>
                </div>
                <span className="bg-secondary/10 text-secondary px-1.5 py-0.5 text-[8px] rounded font-bold uppercase">
                  {marketplace || 'OFFLINE'}
                </span>
              </div>
              ...
              <div className="space-y-0.5">
                ...
                <div>
                  <p className="text-[8px] text-on-surface-variant font-bold uppercase">Nama Pelanggan / Pengirim</p>
              ...
              <div className="pt-1 border-t border-ui-divider">
                ...
                {orderItems.length > 0 ? (
                  <div className="max-h-20 overflow-y-auto space-y-0.5 pr-0.5">
                    {orderItems.map((item, idx) => (
                      <div key={idx} className="flex justify-between items-center bg-surface-container-low px-1.5 py-0.5 rounded text-[10px]">
                        ...
                        <span className="font-bold text-secondary font-mono shrink-0">x{item.qty}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="bg-surface-container-low px-1.5 py-0.5 rounded text-[9px] text-on-surface-variant flex items-center justify-between">
                    ...
                    <span className="font-bold text-secondary">Siap Unboxing</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-surface-container-lowest border border-dashed border-ui-divider rounded-lg p-2 flex flex-col items-center justify-center opacity-70">
              <span className="material-symbols-outlined text-lg mb-0.5 text-on-surface-variant">inventory_2</span>
          ...
          <div className="flex flex-col gap-1 my-auto">
            {scanState === 'SCANNED' && (
              <button 
                ...
                <span>Mulai Rekam Unboxing</span>
              </button>
            )}
            ...
            {scanState === 'RECORDING' && (
              <button 
                ...
                <span>Selesai Unboxing ({formatTime(recordingTime)})</span>
              </button>
            )}
            ...
            {scanState === 'IDLE' && (
              <div className="w-full border border-dashed border-ui-divider rounded-lg flex items-center justify-center text-on-surface-variant font-label-caps text-[10px] py-1.5 bg-surface-container-lowest">
                <span className="material-symbols-outlined mr-1 text-secondary text-sm">center_focus_strong</span>
                SIAP SCAN RESI
              </div>
            )}
          </div>
